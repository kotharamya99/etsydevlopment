...
function EtsyBody() {
  ...
  const getItems = () => {
    Axios.get("http://3.95.28.170:4000/getItems").then((response) => {
  ...
  const getFavourites = () => {
    if (user !== null) {
      Axios.get("http://3.95.28.170:4000/getFavourites/" + user.id).then(
        (response) => {
  ...
  const handleFavourite = (itemId, userId) => {
    ...
    Axios.post("http://3.95.28.170:4000/addFavourite", {
      itemId: itemId,
      userId: userId,
    }).then((response) => {
      if (response.data.success === true) {
      ...
      <div className="home_cards col-md-4 mb-4">
        <div className="home_card card">
          ...
          <div className="card-body">
            ...
            <button className="btn-sm btn-dark" style = {{ borderRadius : "100px"}}>View Product</button>
          </div>
        </div>
      </div>
    );
